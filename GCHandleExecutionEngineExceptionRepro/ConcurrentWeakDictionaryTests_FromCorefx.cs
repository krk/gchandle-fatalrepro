...
namespace GCHandleExecutionEngineExceptionRepro
{
	...
	public class ConcurrentWeakDictionaryTests_FromCorefx
	{
		...
		{
			ConcurrentWeakDictionary<int, int> cd = new ConcurrentWeakDictionary<int, int>();
		...
		public static void TestAdd1()
		{
			...
			TestAdd1<int>(1, 1, 1, 10000, generator, false);
		...
		{
			IDictionary<int, int> dict = new ConcurrentWeakDictionary<int, int>(cLevel, 1);
		...
		{
			ConcurrentWeakDictionary<int, int> dict = new ConcurrentWeakDictionary<int, int>(cLevel, 1);
		...
		{
			ConcurrentWeakDictionary<int, int> dict = new ConcurrentWeakDictionary<int, int>();
		...
		{
			var cd = new ConcurrentWeakDictionary<string, int>(new OrdinalStringComparer());
			cd["test"] = 10;
		...
		{
			var dictionary = new ConcurrentWeakDictionary<int, int>(new[] { new KeyValuePair<int, int>(1, 1) });
		...
		public static void TestConstructor_Negative()
		{
			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>((ICollection<KeyValuePair<int, int>>)null));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when null collection is passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>((IEqualityComparer<int>)null));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when null IEqualityComparer is passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>((ICollection<KeyValuePair<int, int>>)null, EqualityComparer<int>.Default));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when null collection and non null IEqualityComparer passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>(new[] { new KeyValuePair<int, int>(1, 1) }, null));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when non null collection and null IEqualityComparer passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<string, int>(new[] { new KeyValuePair<string, int>(null, 1) }));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when collection has null key passed");
			Assert.Throws<ArgumentException>(
			   () => new ConcurrentWeakDictionary<int, int>(new[] { new KeyValuePair<int, int>(1, 1), new KeyValuePair<int, int>(1, 2) }));
			// "TestConstructor:  FAILED.  Constructor didn't throw AE when collection has duplicate keys passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>(1, null, EqualityComparer<int>.Default));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when null collection is passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>(1, new[] { new KeyValuePair<int, int>(1, 1) }, null));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when null comparer is passed");

			Assert.Throws<ArgumentNullException>(
			   () => new ConcurrentWeakDictionary<int, int>(1, 1, null));
			// "TestConstructor:  FAILED.  Constructor didn't throw ANE when null comparer is passed");

			Assert.Throws<ArgumentOutOfRangeException>(
			   () => new ConcurrentWeakDictionary<int, int>(0, 10));
			// "TestConstructor:  FAILED.  Constructor didn't throw AORE when <1 concurrencyLevel passed");

			Assert.Throws<ArgumentOutOfRangeException>(
			   () => new ConcurrentWeakDictionary<int, int>(-1, 0));
		...
		{
			var dictionary = new ConcurrentWeakDictionary<string, int>();
			Assert.Throws<ArgumentNullException>(
		...
		{
			IDictionary dictionary = new ConcurrentWeakDictionary<string, int>();
		...
		{
			IDictionary<string, int> dictionary = new ConcurrentWeakDictionary<string, int>();
			Assert.Throws<ArgumentNullException>(() => dictionary.Remove(new KeyValuePair<string, int>(null, 0)), "keyValuePair");
		}

		[Test]
		public static void TestICollection()
		{
			ICollection dictionary = new ConcurrentWeakDictionary<int, int>();
			Assert.False(dictionary.IsSynchronized, "TestICollection:  FAILED.  IsSynchronized returned true!");

			int key = -1;
			int value = +1;
			//add one item to the dictionary
			((ConcurrentWeakDictionary<int, int>)dictionary).TryAdd(key, value);
		...
		public static void TestICollection_Negative()
		{
			...
			//add one item to the dictionary
			((ConcurrentWeakDictionary<int, int>)dictionary).TryAdd(1, 1);
		...
		{
			var dictionary = new ConcurrentWeakDictionary<int, int>();
			for (int i = 0; i < 10; i++)
		...
		public static void TestTryUpdate()
		{
			...
			var dict = new ConcurrentWeakDictionary<int, Struct16>();
