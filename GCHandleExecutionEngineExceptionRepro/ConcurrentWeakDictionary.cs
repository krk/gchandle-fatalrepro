...
namespace GCHandleExecutionEngineExceptionRepro
{
    ...
    /// <remarks>
    /// All public and protected members of <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> are thread-safe and may be used
    /// concurrently from multiple threads.
    /// </remarks>
    //[DebuggerTypeProxy(typeof(IDictionaryDebugView<,>))]
    //[DebuggerDisplay("Count = {Count}")]
    public class ConcurrentWeakDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>
    {
        /// <summary>
        /// Tables that hold the internal state of the ConcurrentWeakDictionary
        ///
        ...
        private static bool IsValueWriteAtomic()
        {
            ...
            bool isAtomic = false;
			/*
                !System.Reflection.IntrospectionExtensions.GetTypeInfo(valueType).IsValueType ||
...
*/
        ...
        {
            // Free all GCHandle instances.
            foreach (var node in GetNodes())
            {
                if (node._key.IsAllocated)
                {
                    node._key.Free();
                }

                if (node._value.IsAllocated)
                {
                    node._value.Free();
                }
            }
        ...
        /// cref="ConcurrentWeakDictionary{TKey,TValue}"/>
        /// class that is empty, has the default concurrency level, has the default initial capacity, and
        /// uses the default comparer for the key type.
        /// </summary>
        public ConcurrentWeakDictionary() : this(DefaultConcurrencyLevel, DefaultCapacity, true, EqualityComparer<TKey>.Default) { }
        ...
        /// <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> concurrently.</param>
        ...
        /// 0.</exception>
        public ConcurrentWeakDictionary(int concurrencyLevel, int capacity) : this(concurrencyLevel, capacity, false, EqualityComparer<TKey>.Default) { }

        /// <summary>
        /// Initializes a new instance of the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/>
        ...
        /// the new
        /// <see cref="ConcurrentWeakDictionary{TKey,TValue}"/>.</param>
        /// <exception cref="T:System.ArgumentNullException"><paramref name="collection"/> is a null reference
        /// (Nothing in Visual Basic).</exception>
        /// <exception cref="T:System.ArgumentException"><paramref name="collection"/> contains one or more
        /// duplicate keys.</exception>
        public ConcurrentWeakDictionary(IEnumerable<KeyValuePair<TKey, TValue>> collection) : this(collection, EqualityComparer<TKey>.Default) { }
        ...
        /// (Nothing in Visual Basic).</exception>
        public ConcurrentWeakDictionary(IEqualityComparer<TKey> comparer) : this(DefaultConcurrencyLevel, DefaultCapacity, true, comparer) { }
        ...
        private void InitializeFromCollection(IEnumerable<KeyValuePair<TKey, TValue>> collection)
        {
            ...
            foreach (KeyValuePair<TKey, TValue> pair in collection)
            {
                ...
                {
                    throw new ArgumentException("ConcurrentWeakDictionary_SourceContainsDuplicateKeys");
                }
            }
        ...
        /// <exception cref="T:System.ArgumentNullException"><paramref name="comparer"/> is a null reference
        /// (Nothing in Visual Basic).</exception>
        public ConcurrentWeakDictionary(int concurrencyLevel, int capacity, IEqualityComparer<TKey> comparer)
            : this(concurrencyLevel, capacity, false, comparer)
        {
        }

        internal ConcurrentWeakDictionary(int concurrencyLevel, int capacity, bool growLockArray, IEqualityComparer<TKey> comparer)
        {
            if (concurrencyLevel < 1)
            {
                throw new ArgumentOutOfRangeException(nameof(concurrencyLevel), "ConcurrentWeakDictionary_ConcurrencyLevelMustBePositive");
            }
            if (capacity < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(capacity), "ConcurrentWeakDictionary_CapacityMustNotBeNegative");
            }
            if (comparer == null) throw new ArgumentNullException(nameof(comparer));
        ...
        /// <summary>
        /// Attempts to add the specified key and value to the <see cref="ConcurrentWeakDictionary{TKey,
        /// TValue}"/>.
        /// </summary>
        /// <param name="key">The key of the element to add.</param>
        /// <param name="value">The value of the element to add. The value can be a null reference (Nothing
        /// in Visual Basic) for reference types.</param>
        /// <returns>true if the key/value pair was added to the <see cref="ConcurrentWeakDictionary{TKey,
        /// TValue}"/>
        /// successfully; otherwise, false.</returns>
        /// <exception cref="T:System.ArgumentNullException"><paramref name="key"/> is null reference
        /// (Nothing in Visual Basic).</exception>
        /// <exception cref="T:System.OverflowException">The <see cref="ConcurrentWeakDictionary{TKey, TValue}"/>
        ...
        /// <summary>
        /// Determines whether the <see cref="ConcurrentWeakDictionary{TKey, TValue}"/> contains the specified
        /// key.
        /// </summary>
        /// <param name="key">The key to locate in the <see cref="ConcurrentWeakDictionary{TKey,
        /// TValue}"/>.</param>
        /// <returns>true if the <see cref="ConcurrentWeakDictionary{TKey, TValue}"/> contains an element with
        ...
        /// <see cref="ConcurrentWeakDictionary{TKey, TValue}"/>.
        /// </summary>
        /// <param name="key">The key of the element to remove and return.</param>
        /// <param name="value">When this method returns, <paramref name="value"/> contains the object removed from the
        /// <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> or the default value of <typeparamref
        /// name="TValue"/>
            ...
            while (true)
            {
                ...
                lock (tables._locks[lockNo])
                {
                    ...
                    for (Node curr = tables._buckets[bucketNo]; curr != null; curr = curr._next)
                    {
                        ...
                        if (hashcode == curr._hashcode && _comparer.Equals((TKey)curr._key.Target, key))
                        {
                            if (matchValue)
                            {
                                bool valuesMatch = EqualityComparer<TValue>.Default.Equals(oldValue, (TValue)curr._value.Target);
                                if (!valuesMatch)
                                {
                            ...
                            value = (TValue)curr._value.Target;
        ...
        public bool Trim()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < buckets.Length; i++)
                {
                    ...
                    while (current != null)
                    {
                      bool remove = !current._key.IsAllocated || current._key.Target == null;

                      remove = remove || !current._value.IsAllocated || current._value.Target == null;

                      if (remove)
                      {
                            if (prev == null)
                            {
                                Volatile.Write<Node>(ref buckets[i], current._next);
                            }
                            else
                            {
                            ...
                            var lockNo = i%tables._locks.Length;
        ...
        /// the
        /// <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> with the specified key or the default value of
        /// <typeparamref name="TValue"/>, if the operation failed.</param>
        /// <returns>true if the key was found in the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/>;
        ...
        private bool TryGetValueInternal(TKey key, int hashcode, out TValue value)
        {
            ...
            while (n != null)
            {
                if (hashcode == n._hashcode && _comparer.Equals((TKey)n._key.Target, key))
                {
                    value = (TValue)n._value.Target;
                    return true;
                }
                n = n._next;
            }
            ...
            while (true)
            {
                ...
                lock (tables._locks[lockNo])
                {
                    ...
                    for (Node node = tables._buckets[bucketNo]; node != null; node = node._next)
                    {
                        ...
                        if (hashcode == node._hashcode && _comparer.Equals((TKey)node._key.Target, key))
                        {
                            if (valueComparer.Equals((TValue)node._value.Target, comparisonValue))
                            {
                                var newValueHandle = GCHandle.Alloc(key, GCHandleType.Weak);
                                if (s_isValueWriteAtomic)
                                {
                                    node._value = newValueHandle;
                                }
                                else
                                {
                                    Node newNode = new Node(node._key, newValueHandle, hashcode, node._next);
        ...
        /// <summary>
        /// Removes all keys and values from the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/>.
        /// </summary>
        public void Clear()
        {
            int locksAcquired = 0;
            try
            {
                ...
                _tables = newTables;
                _budget = newTables._buckets.Length / newTables._locks.Length > 1 ? newTables._buckets.Length / newTables._locks.Length : 1;
            }
            finally
            {
                ReleaseLocks(0, locksAcquired);
        ...
        void ICollection<KeyValuePair<TKey, TValue>>.CopyTo(KeyValuePair<TKey, TValue>[] array, int index)
        {
            ...
            if (index < 0) throw new ArgumentOutOfRangeException(nameof(index), "ConcurrentWeakDictionary_IndexIsNegative");
            ...
            try
            {
                ...
                {
                    throw new ArgumentException("ConcurrentWeakDictionary_ArrayNotLargeEnough");
                }
        ...
        /// <summary>
        /// Copies the key and value pairs stored in the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> to a
        /// new array.
        /// </summary>
        /// <returns>A new array containing a snapshot of key and value pairs copied from the <see
        /// cref="ConcurrentWeakDictionary{TKey,TValue}"/>.</returns>
        ...
        private void CopyToPairs(KeyValuePair<TKey, TValue>[] array, int index)
        {
            ...
            for (int i = 0; i < buckets.Length; i++)
            {
                for (Node current = buckets[i]; current != null; current = current._next)
                {
                    ...
                    if (current._key.IsAllocated && current._key.Target == null)
                    {
                        current._key.Free();
                        skip = true;
                    }

                    if (current._value.IsAllocated && current._value.Target == null)
                    {
                        current._value.Free();
                        skip = true;
                    }
                    ...
                    array[index] = new KeyValuePair<TKey, TValue>((TKey)current._key.Target, (TValue)current._value.Target);
        ...
        private void CopyToEntries(DictionaryEntry[] array, int index)
        {
            ...
            for (int i = 0; i < buckets.Length; i++)
            {
                for (Node current = buckets[i]; current != null; current = current._next)
                {
                    ...
                    array[index] = new DictionaryEntry(current._key.Target, current._value.Target);
        ...
        /// cref="ConcurrentWeakDictionary{TKey,TValue}"/>.</summary>
        /// <returns>An enumerator for the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/>.</returns>
        /// <remarks>
        ...
        public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator()
        {
            ...
            for (int i = 0; i < buckets.Length; i++)
            {
                ...
                while (current != null)
                {
                    ...
                    yield return new KeyValuePair<TKey, TValue>((TKey)current._key.Target, (TValue)current._value.Target);
                    current = current._next;
                }
            }
        }
                ...
                try
                {
                    ...
                    for (Node node = tables._buckets[bucketNo]; node != null; node = node._next)
                    {
                        ...
                        if (hashcode == node._hashcode && _comparer.Equals((TKey)node._key.Target, key))
                        {
                            ...
                            if (updateIfExists)
                            {
                                var valueHandle = GCHandle.Alloc(value, GCHandleType.Weak);
                                if (s_isValueWriteAtomic)
                                {
                                    node._value = valueHandle;
                                }
                                else
                                {
                                    Node newNode = new Node(node._key, valueHandle, hashcode, node._next);
                                    if (prev == null)
                                    {
                                        tables._buckets[bucketNo] = newNode;
                                    }
                            ...
                            {
                                resultingValue = (TValue)node._value.Target;
                            }
                            return false;
                        }
                        prev = node;
                    ...
                    var keyHandle = GCHandle.Alloc(key, GCHandleType.Weak);
                    var valueHandle2 = GCHandle.Alloc(value, GCHandleType.Weak);

                    // The key was not found in the bucket. Insert the key-value pair.
                    Volatile.Write<Node>(ref tables._buckets[bucketNo], new Node(keyHandle, valueHandle2, hashcode, tables._buckets[bucketNo]));
                    checked
                    {
                        tables._countPerLock[lockNo]++;
                    }
        ...
        /// cref="ConcurrentWeakDictionary{TKey,TValue}"/>.</value>
        ...
        /// <summary>
        /// Adds a key/value pair to the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> 
        ...
        /// <summary>
        /// Adds a key/value pair to the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> if the key does not already 
        /// exist, or updates a key/value pair in the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> if the key 
        /// already exists.
        /// </summary>
        ...
        /// <summary>
        /// Gets a value that indicates whether the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> is empty.
        /// </summary>
        /// <value>true if the <see cref="ConcurrentWeakDictionary{TKey,TValue}"/> is empty; otherwise,
        /// false.</value>
        public bool IsEmpty
        {
            get
        ...
        /// cref="ConcurrentWeakDictionary{TKey,TValue}"/>.</exception>
        void IDictionary<TKey, TValue>.Add(TKey key, TValue value)
        {
            if (!TryAdd(key, value))
            {
                throw new ArgumentException("ConcurrentWeakDictionary_KeyAlreadyExisted");
            }
        }
        ...
        {
            if (keyValuePair.Key == null) throw new ArgumentNullException(nameof(keyValuePair), "ConcurrentWeakDictionary_ItemKeyIsNull");
        ...
        {
            return ((ConcurrentWeakDictionary<TKey, TValue>)this).GetEnumerator();
        }
        ...
        {
            if (key == null) ThrowKeyNullException();
            if (!(key is TKey)) throw new ArgumentException("ConcurrentWeakDictionary_TypeOfKeyIncorrect");
            ...
            {
                throw new ArgumentException("ConcurrentWeakDictionary_TypeOfValueIncorrect");
            }
        ...
        /// cref="T:System.Collections.Generic.ConcurrentWeakDictionary{TKey,TValue}"/>, this property always
        /// returns false.</value>
        ...
        /// cref="T:System.Collections.Generic.ConcurrentWeakDictionary{TKey,TValue}"/>.</value>
        /// <exception cref="T:System.ArgumentNullException"><paramref name="key"/> is a null reference
        /// (Nothing in Visual Basic).</exception>
        /// <exception cref="T:System.ArgumentException">
        /// A value is being assigned, and <paramref name="key"/> is of a type that is not assignable to the
        /// key type <typeparamref name="TKey"/> of the <see
        /// cref="T:System.Collections.Generic.ConcurrentWeakDictionary{TKey,TValue}"/>. -or- A value is being
        /// assigned, and <paramref name="key"/> is of a type that is not assignable to the value type
        /// <typeparamref name="TValue"/> of the <see
        /// cref="T:System.Collections.Generic.ConcurrentWeakDictionary{TKey,TValue}"/>
        /// </exception>
        object IDictionary.this[object key]
        {
            ...
            set
            {
                ...
                if (!(key is TKey)) throw new ArgumentException("ConcurrentWeakDictionary_TypeOfKeyIncorrect");
                if (!(value is TValue)) throw new ArgumentException("ConcurrentWeakDictionary_TypeOfValueIncorrect");

                ((ConcurrentWeakDictionary<TKey, TValue>)this)[(TKey)key] = (TValue)value;
            }
        }
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            try
            {
                ...
                throw new ArgumentException("ConcurrentWeakDictionary_ArrayIncorrectType", nameof(array));
            }
            finally
            {
                ReleaseLocks(0, locksAcquired);
        ...
        /// cref="T:System.Collections.Concurrent.ConcurrentWeakDictionary{TKey,TValue}"/>, this property always
        /// returns false.</value>
        ...
        private void GrowTable(Tables tables)
        {
            ...
            try
            {
                ...
                if (maximizeTableSize)
                {
                    ...
                    // calling Clear() on the ConcurrentWeakDictionary will shrink the table and lower the budget.)
                    _budget = int.MaxValue;
                }
                ...
                // Adjust the budget
                _budget = newBuckets.Length / newLocks.Length > 1 ? newBuckets.Length / newLocks.Length : 1;
        ...
        private void AcquireAllLocks(ref int locksAcquired)
        {
            ...
            {
                CDSCollectionETWBCLProvider.Log.ConcurrentWeakDictionary_AcquiringAllLocks(_tables._buckets.Length);
            }
#endif
            ...
            try
            {
                ...
                for (int i = 0; i < _tables._buckets.Length; i++)
                {
                    ...
                    while (current != null)
                    {
                        ...
                        keys.Add((TKey)current._key.Target);
                        current = current._next;
                    }
                }
            ...
            try
            {
                ...
                for (int i = 0; i < _tables._buckets.Length; i++)
                {
                    ...
                    while (current != null)
                    {
                        ...
                        values.Add((TValue)current._value.Target);
                        current = current._next;
                    }
                }
        ...
        {
            internal readonly GCHandle _key;
            internal GCHandle _value;
            internal volatile Node _next;
            internal readonly int _hashcode;

            internal Node(GCHandle key, GCHandle value, int hashcode, Node next)
            {
                _key = key;
                _value = value;
                _next = next;
        ...
        private sealed class DictionaryEnumerator : IDictionaryEnumerator
        {
            ...
            internal DictionaryEnumerator(ConcurrentWeakDictionary<TKey, TValue> dictionary)
            {
